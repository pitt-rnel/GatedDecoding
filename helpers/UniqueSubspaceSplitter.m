function [Subspaces,out] = UniqueSubspaceSplitter(Xcell,var_cutoff,varargin)
% Splits data into orthogonal subspaces containing condition-unique and 
% condition-shared activity.
%   [Subspaces] = DekODec(Xcell,var_cutoff)
%
%- INPUTS -----------------------------------------------------------------
%
...
%             t_uniqueXvar = var(i_in_noi*unique_i)./tivar*100; % explained var
            % Only keep dimensions that explain at least as much variance as the
            % last PC when calculated on the full condition data
%             prospective_dim = scree_dimensionality(uniqueXvar); 
%             dimUis{j} = find(~fliplr(cumsum(fliplr(uniqueXvar)) < (100-var_cutoff)),1,'last'); 
%             dimUis{j} = find(~fliplr(cumsum(fliplr(t_uniqueXvar)) < (100-var_cutoff)),1,'last'); 
            dimUis{j} = find(uniqueXvar >= evXs{j}(dimX(j)+1),1,'last');  
            
            Cvi = cov(i_in_noi*unique_i); 
            
%             dimUis{j} = sum(diag(Cvi)>(trace(Cvi)/size(i_in_noi,2)) & (diag(Cvi)./trace(cov(Xcell{combi{z}(i,j)}))*100) >= evXs{j}(dimX(j)+1)); 
            dimUis{j} = sum((diag(Cvi)./trace(cov(Xcell{combi{z}(i,j)}))*100) >= evXs{j}(dimX(j)+1)); 
